...
module.exports.run = async (bot, message, args) => {
  ...
  let dollor = await db.fetch(`dollar_${user.id}`)
  if (dollor === null) dollor = 0;

  
  let pound = await db.fetch(`pound_${user.id}`)
  if (pound === null) pound = 0;

  
  let gold = await db.fetch(`gold_${user.id}`)
  if (gold === null) gold = 0;

  
  let silver = await db.fetch(`silver_${user.id}`)
...
let mbd = await db.fetch(`bd_${user.id}`)
  ...
  let developer = await db.fetch(`bd_${user.id}`)
    ...
    if(prt === true) rank = '<:par:750306197445410866> DB PARTNER '
    if(developer === true) rank = '<:theDiscordBoxlogo:778851015445315614> DB DEVELOPER'
  ...
  const card = `https://DiscordBoxplus.sirv.com/credit_card_PNG106.png?text.0.text=${money}&text.0.position.x=-40%25&text.0.position.y=-64%25&text.0.size=15&text.0.color=000000&text.1.text=${bank}&text.1.position.x=-40%25&text.1.position.y=-29%25&text.1.size=15&text.1.color=000000&text.2.text=${encodeURIComponent(member.user.username)}+|+&text.2.position.x=-27%25&text.2.position.y=-3%25&text.2.align=right&text.2.size=26&text.2.color=000000&text.2.font.weight=600`
  ...
  .setDescription(`**مقدار دارایی ${user}**`)

  .addField(`<:Pvoin:778188788522418176>  پیکوین: `, `${money}`, true)
  .addField(`<:pbank:811331113028288512>  بانک: `, `${bank}`, true)
  .addField(`<:money:778590631307771936> دلار: `, `${dollor}`, true)
  .addField(`<:money:778590631307771936> پوند: `, `${pound}`, true)
  .addField(`<:gold:778591355575730186>  طلا: `, `${gold}`, true)
  .addField(`<:silver:778591359363710976> نقره: `, `${silver}`, true)
