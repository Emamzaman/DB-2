...
.setColor("#f2ecec")
.setDescription(`شما نمی توانید از خود دزدی کنید  **|** ❌    `);
...
if (weekly !== null && timeout2 - (Date.now() - weekly) > 0) {
  ...
  .setColor("#f2ecec")
  .setDescription(`شخص مورد نظر آیتم ضد دزدی دارد **|** ❌    `);
...
if (author !== null && timeout - (Date.now() - author) > 0) {
    ...
    let timeEmbed = new Discord.MessageEmbed()
    .setColor("#f2ecec")
    .setDescription(`همین الان دزدی صورت گرفته و شما قادر به دزدی دیگر را تا ${time.hours}h ${time.minutes}m ${time.seconds}s ساعت دیگر را ندارید  **|** ❌    `);
...
.setColor("#f2ecec")
.setDescription(`.${args[0]}. **.1.** پیکوین ندارد  **|** ❌    `)
...
.setColor("#f2ecec")
.setDescription(`شما حداقل نیاز به **.1.** پیکوین برای حمله به یک ممبر دیگر دارید  **|** ❌    `)
.addField("اگر پیکوین لازم را ندارید از این دستور استفاده کنید", "`!!makepcoin`");
