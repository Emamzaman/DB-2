...
module.exports.run = async (bot, message, args) => {
  ...
  let private = await db.fetch(`private_${user.id}`)

  if(member11 !== member22 && private == true) {
    let privatewmbed = new Discord.MessageEmbed()
    .setColor("#f2ecec")
    .setDescription(`این شخص اطلاعات اکانت خود را برای دیگران بسته است  **|** ❌    `);
    
    message.channel.send(privatewmbed);
   } else {
...
let mbd = await db.fetch(`bd_${user.id}`)
    ...
    if(mbh === true) medalbh = ' <:theDiscordBoxlogo:778851015445315614> **|**'
    if(mprt === true) medalprt = ' <:theDiscordBoxlogo:778851015445315614> **|**'
    ...
    if(mbd === true) medalbd = ' <:theDiscordBoxlogo:778851015445315614>  **|**'
  ...
  let developer = await db.fetch(`bd_${user.id}`)
    ...
    if(bh === true) rank = '<:theDiscordBoxlogo:778851015445315614> Technician'
    if(prt === true) rank = '<:theDiscordBoxlogo:778851015445315614> PARTNER '
    if(developer === true) rank = '<:theDiscordBoxlogo:778851015445315614> DEVELOPER'
  ...
  let rob = await db.fetch(`${user.id}_anti-rob-icon`)
  if(rob === null) rob = '<:off:784381666843557889>'

  let attack = await db.fetch(`${user.id}_anti-attack-icon`)
  if(attack === null) attack = '<:off:784381666843557889>'

  let doble2 = await db.fetch(`${user.id}_doble2`)
  if(doble2 === null) doble2 = '<:off:784381666843557889>'

  let timeout = 604800000;
 
  let rob2 = await db.fetch(`${user.id}_anti-rob`)
  let attack2 = await db.fetch(`${user.id}_anti-attack`)
  let doble22 = await db.fetch(`${user.id}_doble2`)


  let robtime1 = ms(timeout - (Date.now() - rob2));
  let robtime = `${robtime1.days}d ${robtime1.hours}h ${robtime1.minutes}m ${robtime1.seconds}s`
  if(rob2 === null) robtime = 'غیرفعال'

  let attacktime1 = ms(timeout - (Date.now() - attack2));
  let attacktime = `${attacktime1.days}d ${attacktime1.hours}h ${attacktime1.minutes}m ${attacktime1.seconds}s`
  if(attack2 === null) attacktime = 'غیرفعال'

  let doble2time1 = ms(timeout - (Date.now() - doble22));
  let doble2time = `${doble2time1.days}d ${doble2time1.hours}h ${doble2time1.minutes}m ${doble2time1.seconds}s`
  if(doble22 === null) doble2time = 'غیرفعال'

  let allmoney = money+bank
  ...
  .setDescription(`**اطلاعاتِ ${user}**`)
  .addField(`کل دارایی`, `${allmoney}`)
  .addField(`آیتم ضد دزدی ${rob}`, robtime)
  .addField(`آیتم ضد حمله ${attack}`, attacktime)
  .addField(`آیتم دو برابر ${doble2}`, doble2time)
  .addField('ولپیپر', `[Link](${wallpaper})`)
  .addField('رنگ', `${color}`)
