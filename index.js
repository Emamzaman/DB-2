...
setInterval(function() {
  ...
  bot.user.setActivity(`The DiscordBox | The First & Best Iranian Economy Bot`, { type: 'PLAYING' });
  bot.user.setActivity(`The DiscordBox | Ping : ${fackeping}`, { type: 'WATCHING' });
  bot.user.setActivity(`The DiscordBox | ${bot.guilds.cache.size} Servers`, { type: 'WATCHING' });
  bot.user.setActivity('The DiscordBox | !!help', { type: 'LISTENING' });
}, 5000)
});
...
bot.on("message", async message => {
    ...
    let DiscordBoxprefix = db.fetch(`${message.guild.id}prefix`) || "lljjjiygkj215";

    let msgcoin = db.fetch(`${message.guild.id}msgcoin`) || "1";
    
    let prefix = DiscordBoxprefix;
...
bot.on("message", async message => {
    ...
    let DiscordBoxprefix = PF6Config.prefix;
